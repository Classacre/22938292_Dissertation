...
#      two independent methylation datasets (Cahn et al., and Bewick et al.),
#      specifically for the 'gbM' classification, using a Venn diagram.
#
#   2. Single-Cell Data Overview: It provides a high-level summary and
#      visualization of the unpublished Seurat dataset, including a UMAP plot,
#      cell counts, and data dimensions.
#
# DISCLAIMER:
...
gbm_gene_lists <- list(
  `Cahn et al.` = cahn_gbm_genes,
  `Bewick et al.` = bewick_gbm_genes
)
...
p_umap <- DimPlot(seurat_obj, reduction = "umap.rpca", group.by = "identity", label = TRUE, repel = TRUE) +
  labs(title = "UMAP of Cell Types",
       subtitle = "Clusters include cell type and ERS status") +
...
p_counts <- ggplot(cell_counts, aes(x = reorder(CellType, -Count), y = Count, fill = CellType)) +
  ...
  labs(title = "Cell Counts per Identity", x = "Cell Identity", y = "Number of Cells") +
  theme_bw(base_size = 12) +
...
p_seurat_overview <- p_umap + p_counts +
  plot_annotation(
    ...
    subtitle = "Unpublished data from Oliva et al., used for expression and noise analysis",
...
summary_text <- c(
  ...
  paste("Number of identified identities (cell type + ERS state):", length(unique(seurat_obj$identity))),
  "",
...
message("Script 00 finished successfully.")