...
**The main analysis pipeline is split into modular, sequentially numbered R scripts for maintainability and clarity.**

To run the full pipeline, use the SLURM batch file:

```sh
sbatch run_gbm_noise_pipeline.sh
```


## Files and Inputs
- **Seurat object:** `/group/sms029/Oliva_dataset/integrated_col_trajectories_colonly.rds`
- **Combined methylation annotation:** `/group/sms029/mnieuwenh/Methylation_Data/01_combined_methylation_data.csv` (generated by the pipeline)
...
## Main Steps (Automated by `run_gbm_noise_pipeline.sh`)

1. **Combine methylation datasets**
   - `01_combine_methylation_datasets.R`: Merges Cahn et al. and Bewick et al. methylation data, adds H2A.Z annotation, outputs `01_combined_methylation_data.csv`.
2. **Export data from Seurat**
   - `02_export_data.R`: Exports log-normalized gene expression matrix and cell metadata from the Seurat object.
3. **Calculate gene expression noise**
   - `03_calculate_noise.R`: Calculates raw and corrected noise metrics, identifies highly variable genes, and generates summary statistics and plots.
4. **Create master gene summary**
   - `04_create_master_gene_summary.R`: Integrates noise, HVG, responsiveness, and epigenetic annotations into a single summary table.
5. **Analyze responsive genes**
   - `05_analyze_responsive_genes.R`: Identifies cell-type-specific (responsive) genes and compares their noise and epigenetic features.
6. **Subsampling validation**
   - `06_subsampling_validation.R`: Performs mean-expression-matched subsampling to validate noise metric independence from mean expression.
7. **Visualize filtering**
   - `07_visualize_filtering.R`: Generates diagnostic plots showing the effect of gene filtering steps.
8. **Supervisor requests/diagnostics**
   - `08_supervisor_requests.R`: Additional diagnostics and correlation plots for supervisor review.


## How to Run

To run the entire pipeline (all steps above) in order:

```sh
sbatch run_gbm_noise_pipeline.sh
```

This will execute all R scripts in sequence, producing all outputs and summary tables in the appropriate subfolders.


## Output Files
- **Combined methylation annotation:** `../Methylation_Data/01_combined_methylation_data.csv`
- **Expression matrix:** `02_expression_matrix.csv`
- **Cell metadata:** `02_seurat_metadata_full.csv`
- **Noise analysis results:** `results/03_noise_analysis/noise_analysis_complete_data.csv`
- **Master gene summary:** `results/04_master_summary/master_gene_summary_table.csv`
- **Responsive gene analysis:** `results/05_responsive_gene_analysis/`
- **Subsampling validation:** `results/06_subsampling_validation/`
- **Filtering diagnostics:** `results/07_diagnostic_plots/`
- **Supervisor diagnostics:** `results/08_supervisor_requests/`

All plots, tables, and summary statistics are saved in their respective results subfolders.

## Batch Scripts
- All batch scripts (`*.sh`) are provided for SLURM job submission and reproducibility.
- The main analysis pipeline is now run with `run_gbm_noise_pipeline.sh`.
...
## Notes
- All statistical results are summarized in a single table and summary text file per analysis folder for clarity.
- The pipeline supports flexible comparison of gene expression noise and responsiveness across methylation and H2A.Z categories, including their combinations.
- The pipeline is modular, with each R script handling a specific step.
- All code and workflow steps are provided for transparency and reproducibility.
